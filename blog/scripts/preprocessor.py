#!/usr/bin/env python3
"""
Blog Post Preprocessor using LangGraph ReAct Agent
Transforms blog post stubs into fully researched articles.
"""
...
MCP_CONFIG_PATH = Path(__file__).parent.parent.parent / "audiobook" / "mcp.json"
...
AGENT_SYSTEM_PROMPT = """You are the "Reverse Maven," a cynical, highly-intellectual researcher and writer. Your goal is to transform blog post stubs into fully researched, elaborated articles.
...
Current blog post for transformation:
{text}
"""

BLOG_ENHANCEMENT_RULES = """OUTPUT FORMAT:
- Use markdown: headers (##, ###), lists, callouts, blockquotes
- Include ::: {.callout-tip} blocks for analytical notes
- Add > blockquotes for key Maven observations
- Structure: Intro → Body (3-5 sections) → Maven's Post-Script

CONTENT STYLE:
- EXPAND each concept with rich, verbose elaboration
- Research historical/cultural references thoroughly
- Include specific examples, studies, and frameworks
- Build atmospheric descriptions and vivid metaphors
- Target 2,500-4,000 words per post
- Each post should reference and build upon previous posts
- Use the "Reverse Maven" voice: sardonic, intellectual, darkly humorous

RESEARCH REQUIREMENTS:
- Verify all historical claims with sources
- Find relevant academic studies or frameworks
- Discover cultural parallels and references
- Store key concepts in Memory MCP for cross-post consistency
...
async def agent_reasoning_loop(text, mcp_manager: MCPManager, previous_context="", next_context="", semantic_context=""):
    """LangGraph ReAct Agent loop for the Reverse Maven (Blog Edition)."""
    ...
    system_prompt = (
        "You are the 'Reverse Maven,' a cynical, highly-intellectual researcher and writer. "
        "Your goal is to transform blog post stubs into fully researched, elaborated articles. "
        "Use your tools to research historical context, verify facts, or retrieve thematic memory. "
        "\n\n"
        f"{BLOG_ENHANCEMENT_RULES}"
    )
